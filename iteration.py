...
def fix_wave(current_wave, remaining_enemies, current_enemies, current_status, timer, spawn_time):
...
def manage_enemy_actions(current_enemies, hero, screen, timer, spawn_time):
    for enemy in current_enemies:
        ...
        if (timer - spawn_time[enemy]) % enemy.attack_speed == 0:
...
def manage_collisions(hero, current_enemies):
    ...
    for enemy in current_enemies:
        if manage_body_colision(hero, enemy, current_enemies):
            return True
        if manage_melee_colision(hero, enemy, current_enemies):
            return True
...
def process_game_iteration(hero, screen, current_status, current_wave,
    ...
    temp_result = fix_wave(current_wave, remaining_enemies, current_enemies, current_status, timer, spawn_time)
    current_wave, remaining_enemies, current_enemies, current_status = temp_result
    
    manage_enemy_actions(current_enemies, hero, screen, timer, spawn_time)
